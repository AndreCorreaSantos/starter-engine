...
<stdin>:19:5: error: Operation 'calyx.repeat' not supported for FSM compilation
    scf.for %arg0 = %c0 to %c3 step %c1 {
    ^
<stdin>:19:5: note: see current operation: 
...
// -----// IR Dump After CalyxToFSM Failed (lower-calyx-to-fsm) //----- //
calyx.component @main(%clk: i1 {clk}, %reset: i1 {reset}, %go: i1 {go}) -> (%done: i1 {done}) {
  %c1_i32 = hw.constant 1 : i32
  %true = hw.constant true
  %false = hw.constant false
  ...
  calyx.control {
    fsm.machine @control_main() attributes {initialState = "fsm_entry"} {
      ...
      fsm.state @seq_0 output {
        fsm.output
      } transitions {
      }
      fsm.state @seq_0_seq_7 output {
        fsm.output
      } transitions {
      }
    }
